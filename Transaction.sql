...
-- 사용자는 아이디를 입력하고 중복확인을 함 만약 입력한 아이디가 중복된 아이디가 아니라면
-- 비밀번호, 비밀번호 확인, 이메일을 입력하고 이메일로 인증 번호를 전송
...
# TCL 트랜잭션 컨트롤 랭귀지라고도 불림
# 내가 하고있는 트랜잭션이 다른사람에 영향을 주면 안됨

# 연산자 추가
-- LIMIT 개수 : 결과테이블에서 지정한 개수만큼의 상위 데이터만 보여줌
SELECT * FROM jeju LIMIT 10;	
-- LIMIT 제외레코드개수, 개수 : 상위에서 제외 레코드 개수만큼 제외 후 개수만큼의 데이터만 보여줌 
