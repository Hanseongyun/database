...
-- NOT NULL 제약조건 : 해당 컬럼에 null값을 포함하지 못하도록 함
...
CREATE TABLE composite_table (
    ...
    primary1 INT,
    primary2 INT,
    CONSTRAINT primary_key PRIMARY KEY(primary1, primary2)	# CONSTRAINT 이름
);
...
-- FOREIGN KEY: 특정 칼럼을 다른 테이블 혹은 같은 테이블의 기본키 컬럼과 연결
...
-- FOREIGN KEY 제약조건이 걸린 컬럼에 INSERT 작업시 참조하고있는 테이블의 컬럼에 값이 존재하지 않으면 지정 할 수 없음
INSERT INTO foreign_table VALUES (1, 1);
INSERT INTO foreign_table VALUES (2, 3);			# 참조하고있는 primary_column에 3이없으므로
# Cannot add or update a child row: a foreign key constraint fails

-- FOREIGN KEY 제약조건이 걸린 컬럼에 UPDATE 작업시 참조하고있는 테이블의 컬럼에 값이 존재하지 않으면 지정 할 수 없음
...
-- CHECK로 지정된 컬럼은 지정 조건에 부합하지 않으면 UPDAETE 불가능
