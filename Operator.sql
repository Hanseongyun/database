...
-- OR (||): 좌항과 우항중 하나라도 ture 이면 true
SELECT * FROM jeju WHERE speed_80m > 4 OR direction_50m < 180;

-- XOR : 좌항과 우항이 다르면 true (AND 갯수 - OR 갯수, 합집합 - 교집합)
...
-- % : 임의의 개수 (0 ~ 무한대)의 문자
-- _ : 임의의 한 개 문자
SELECT * FROM jeju WHERE observe_date LIKE '20%';		# 20으로 시작하는
SELECT * FROM jeju WHERE observe_date LIKE '%08';		# 08로 끝나는 문자열
SELECT * FROM jeju WHERE observe_date LIKE '%08%';		# 08을 포함하는 모든 데이터
...
SELECT observe_date FROM jeju ORDER BY speed_80m DESC;		# 1.FROM 2.WHERE 3.ORDER BY 4.SELECT 순서대로
